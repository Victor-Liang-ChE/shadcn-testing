...
import { ChevronUp, ChevronDown, ChevronsUpDown, BarChart2, Table2, LayoutGrid, Search, ArrowLeft, User, Loader2, Activity } from 'lucide-react';
...
echarts.use([TooltipComponent, GridComponent, MarkLineComponent, LineChart, BarChart, CanvasRenderer]);
...
interface EloData {
  ...
  allFighters?: { name: string; currentElo: number; primaryWeightClass: string; lastFightDate: string }[];
}

type SortCol = 'currentElo' | 'peakElo' | 'peakDate' | 'fights' | 'l5Wins' | 'streak' | 'wl';
...
const PALETTE = [
  ...
  '#67e0e3', '#e690d1', '#e7bcf3', '#8378ea', '#96dee8',
];
...
const DIVISION_ORDER = [
  ...
  'women_featherweight', 'women_bantamweight', 'women_flyweight', 'women_strawweight',
];
...
function computeFighterL5Streak(f: FighterSummary): { l5W: number; l5L: number; l5Str: string; streak: number } {
  const sorted = [...f.history].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
...
function formatDob(dob: string): string {
  ...
  if (today.getMonth() < d.getUTCMonth() || (today.getMonth() === d.getUTCMonth() && today.getDate() < day)) age--;
...
export default function UfcEloPage() {
  ...
  const [viewMode, setViewMode] = useState<'both' | 'chart' | 'table' | 'distribution'>('both');
  ...
  const [championMap, setChampionMap] = useState<Map<string, ChampionEntry>>(new Map());
  const [candidateTott, setCandidateTott] = useState<Map<string, { height?: string|null; weight?: string|null; reach?: string|null; stance?: string|null; dob?: string|null }>>(new Map());
  const [candidateHeadshots, setCandidateHeadshots] = useState<Map<string, string>>(new Map());
  ...
  const handleChartMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    } catch { cursorDataYRef.current = null; }
  }, []);

  const handleChartMouseLeave = useCallback(() => { cursorDataYRef.current = null; }, []);
  ...
  useEffect(() => {
    fetch('/api/ufc-champions/lineage')
      ...
      .then((json) => {
        ...
        for (const [, wc] of Object.entries(json.lineage as Record<string, { champions: ChampionEntry[] }>)) {
          for (const champ of wc.champions) {
            const isInterim = champ.notes?.toLowerCase().includes('interim') ?? false;
  ...
  useEffect(() => {
    if (!data) return;
    const nk = (s: string) => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[.'\u2011]/g, '').toLowerCase().trim();
    ...
    fetch('/api/ufc-tott')
      ...
      .then(json => {
        if (!json?.data) return;
        const map = new Map<string, { height?: string|null; weight?: string|null; reach?: string|null; stance?: string|null; dob?: string|null }>();
        for (const r of json.data as any[]) {
          map.set(nk(r.FIGHTER), { height: r.HEIGHT, weight: r.WEIGHT, reach: r.REACH, stance: r.STANCE, dob: r.DOB });
        }
    ...
    supabase.from('ufc_champion_images')
      .select('FIRST, LAST, HEADSHOT')
      .limit(2000)
      .then(({ data: rows }) => {
        if (!rows) return;
        const map = new Map<string, string>();
        for (const r of rows as any[]) { if (r.HEADSHOT) map.set(nk(`${r.FIRST} ${r.LAST}`), r.HEADSHOT); }
  ...
  useEffect(() => {
    if (viewMode !== 'both') {
      ...
      const layoutMain = document.querySelector<HTMLElement>('body > div > main');
  ...
  useEffect(() => { setLockedFighter(null); }, [selectedDivision]);
  ...
  const candidatePool = useMemo(() => {
    if (!data) return [];
    const base = selectedDivision === 'overall' ? data.candidatePool : (data.byWeightClass[selectedDivision] || []);
  ...
  const eloHistoryMap = useMemo(() => {
    ...
    const add = (f: FighterSummary) => {
      const key = f.name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[.'\u2011]/g, '').toLowerCase().trim();
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const [{ data: prefixData }, { data: containsData }] = await Promise.all([
          supabase.from('ufc_fighter_tott').select('FIGHTER').ilike('FIGHTER', `${q}%`).limit(100),
          supabase.from('ufc_fighter_tott').select('FIGHTER').ilike('FIGHTER', `%${q}%`).limit(300),
        ]);
        ...
        for (const row of [...(prefixData ?? []), ...(containsData ?? [])]) {
          ...
          if (!seen.has(n)) { seen.add(n); allData.push(row as { FIGHTER: string }); }
        }
        if (allData.length === 0) { setSearchResults([]); return; }
        ...
        scored.sort((a: any, b: any) => b._score - a._score || a.FIGHTER.localeCompare(b.FIGHTER));
        const fightersData = scored.slice(0, 14);

        // Fetch headshots for matched fighters — include accent-stripped variants of first words
        const stripA = (s: string) => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim();
        const firstWords = [...new Set(fightersData.slice(0, 50).flatMap((f: any) => {
        ...
        if (imagesData) {
          for (const img of imagesData as any[]) {
            if (img.HEADSHOT) imageMap.set(normKey(`${img.FIRST} ${img.LAST}`), img.HEADSHOT);
          }
        }

        const buildResult = (name: string, hist: typeof eloHistoryMap extends Map<string, infer V> ? V : never): SearchResult => {
          ...
          hist.forEach(h => { if (h.weightClass) wcCounts.set(h.weightClass, (wcCounts.get(h.weightClass) ?? 0) + 1); });
          const topWc = [...wcCounts.entries()].sort((a, b) => b[1] - a[1])[0]?.[0];
          return {
            name,
        ...
        if (toFetch.length > 0) {
          const promises = toFetch.map(async (r) => {
            try {
              const res = await fetch(`/api/ufc-elo?name=${encodeURIComponent(r.name)}`);
              if (!res.ok) return { name: r.name, status: 'not_found' as const };
              const d = await res.json();
              const hist: Array<{ date: string; elo: number; event: string; result: string; weightClass: string }> = d.history || [];
              if (hist.length === 0) return { name: r.name, status: 'not_found' as const };
              const wins = hist.filter(h => h.result === 'W').length;
              const losses = hist.filter(h => h.result === 'L').length;
              const draws = hist.filter(h => h.result === 'D').length;
              const peakElo = Math.max(...hist.map(h => h.elo));
            ...
            } catch { return { name: r.name, status: 'not_found' as const }; }
          });
          for (const p of promises) {
            p.then(result => {
              ...
              setSearchResults(prev => prev.map(r => {
                ...
                if (result.status === 'not_found') return { ...r, eloStatus: 'not_found' as const };
                return {
                  ...r,
      ...
      } catch (e) { console.error('Fighter search error:', e); }
      finally { if (searchIdRef.current === searchId) setIsSearching(false); }
    }, 350);
    return () => clearTimeout(timer);
  }, [searchQuery, eloHistoryMap]);

  // Enrich the pool with precomputed L5/streak so we can sort by them
  const enrichedPool = useMemo(() => candidatePool.map((f) => ({ ...f, ...computeFighterL5Streak(f) })), [candidatePool]);
  ...
  const displayFighters = useMemo(() => {
    const sorted = [...enrichedPool].sort((a, b) => {
      ...
      if (sortCol === 'currentElo') { vA = a.currentElo; vB = b.currentElo; }
      else if (sortCol === 'peakElo') { vA = a.peakElo; vB = b.peakElo; }
      else if (sortCol === 'peakDate') {
        vA = a.peakDate ? new Date(a.peakDate).getTime() : 0;
        vB = b.peakDate ? new Date(b.peakDate).getTime() : 0;
      } else if (sortCol === 'l5Wins') { vA = a.l5W; vB = b.l5W; }
      else if (sortCol === 'streak') { vA = a.streak; vB = b.streak; }
      else if (sortCol === 'wl') {
        ...
        vA = getRatio(a); vB = getRatio(b);
      }
      else { vA = a.fights; vB = b.fights; }
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setTableRowHeight(Math.max(28, Math.floor(available / displayFighters.length)));
    };
    update();
  ...
  const handleSort = (col: SortCol) => {
    ...
    else { setSortCol(col); setSortDir(col === 'peakDate' ? 'asc' : 'desc'); }
  };
  ...
  const pointMeta = useMemo(() =>
    displayFighters.map((fighter) =>
      fighter.history
        ...
        .map((h) => ({ event: h.event, opponent: h.opponent, result: h.result, date: h.date, url: h.url }))
    )
  , [displayFighters]);
    ...
    click: (params: any) => {
      if (params.componentType === 'series' && params.componentSubType === 'line') {
        ...
        if (params.seriesName && params.seriesName !== '__baseline__') {
          ...
          setTimeout(() => setLockedFighter((prev) => (prev === sn ? null : sn)), 0);
        }
        ...
        try {
          if (params.data && Array.isArray(params.data) && params.data[0]) {
            ...
            if (fights) {
              ...
              const fight = fights.find((f) => new Date(f.date).getTime() === clickTs);
              if (fight?.url) {
  ...
  const SortArrow = ({ col }: { col: SortCol }) => {
    if (sortCol !== col) return <ChevronsUpDown className="inline w-3 h-3 ml-0.5 opacity-40" />;
    return sortDir === 'desc'
  ...
  const chartOption: EChartsOption = useMemo(() => {
    ...
    const series = displayFighters.map((fighter, i) => {
      ...
      const originalLastTs = points.length > 0 ? points[points.length - 1][0] : 0;
    ...
    const allHistoryTs = displayFighters.flatMap((f) =>
      f.history.filter((h) => h.date && !isNaN(new Date(h.date).getTime())).map((h) => new Date(h.date).getTime())
    );
    const baselineMinTs = allHistoryTs.length ? Math.min(...allHistoryTs) - 1 : todayTs - 10 * 365.25 * 24 * 60 * 60 * 1000;
    const baselineSeries = {
      name: '__baseline__',
      type: 'line' as const,
      data: [[baselineMinTs, 1500], [todayTs, 1500]] as [number, number][],
    ...
    const historySnapshot = displayFighters.map((f) => {
      const history = f.history
        ...
        .map((h, j, arr) => ({
          ...
          elo: h.elo,
          prevElo: h.eloBeforeFight !== undefined ? h.eloBeforeFight : (j > 0 ? arr[j - 1].elo : 1500),
          event: h.event,
        }));
      const originalLastTs = history.length > 0 ? history[history.length - 1].ts : 0;
    ...
    return {
      ...
      tooltip: {
        ...
        axisPointer: {
          type: 'line',
          lineStyle: { color: isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.25)', type: 'dashed' as const, width: 1 },
        },
        ...
        formatter: (params: any) => {
          ...
          const isGhost = historySnapshot.some(({ history }) => history.some(h => h.ts - 1 === cursorTs));
          if (isGhost) return null as any;

          const date = new Date(cursorTs).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });

          const rows: { name: string; elo: number; color: string; delta: number | null; fightEvent?: string }[] = [];
          historySnapshot.forEach(({ name, history, originalLastTs, isExtended }, i) => {
            ...
            rows.push({ name, elo: lastElo, color: PALETTE[i % PALETTE.length], delta, fightEvent });
          });
      ...
      grid: { top: 8, bottom: viewMode === 'chart' ? 50 : 28, left: '4%', right: '40px', containLabel: true },
      xAxis: {
        type: 'time',
        axisLine: { show: true, lineStyle: { color: textColor } },
        axisTick: { show: true, lineStyle: { color: textColor }, length: 5, inside: false },
        axisLabel: { color: textColor, fontSize: 11, fontFamily: 'Merriweather Sans' },
  ...
  const allFightersForDist = useMemo(() => {
    ...
    const pool = selectedDivision === 'overall' ? all : all.filter(f => f.primaryWeightClass === selectedDivision);
    if (!activeOnly) return pool;
    const cutoff = Date.now() - TWO_YEARS_MS;
    return pool.filter(f => f.lastFightDate && new Date(f.lastFightDate).getTime() >= cutoff);
  }, [data, selectedDivision, activeOnly]);

  const distStats = useMemo(() => {
    if (allFightersForDist.length === 0) return null;
    const elos = [...allFightersForDist.map(f => f.currentElo)].sort((a, b) => a - b);
    const n = elos.length;
    const mean = elos.reduce((a, b) => a + b, 0) / n;
    const median = n % 2 === 0 ? (elos[n / 2 - 1] + elos[n / 2]) / 2 : elos[Math.floor(n / 2)];
    const variance = elos.reduce((sum, x) => sum + (x - mean) ** 2, 0) / n;
    const std = Math.sqrt(variance);
    const q1 = elos[Math.floor(n * 0.25)];
    const q3 = elos[Math.floor(n * 0.75)];
    return { n, mean, median, std, q1, q3, min: elos[0], max: elos[n - 1], elos };
  }, [allFightersForDist]);
  ...
  const distChartOption = useMemo((): EChartsOption => {
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          const bar = Array.isArray(params) ? params.find((p: any) => p.seriesName === 'Count') : null;
          if (!bar) return '';
          const center = bar.data[0] as number;
          const lo = center - BIN / 2;
          const hi = center + BIN / 2;
          return `<span style="font-weight:600">${lo}–${hi}</span><br/>Fighters: <b>${bar.data[1]}</b>`;
        },
      },
      xAxis: {
        type: 'value',
        ...
        axisLabel: { color: textColor, fontSize: 11, fontFamily: 'Merriweather Sans' },
        axisLine: { show: true, lineStyle: { color: isDark ? '#ffffff' : '#000000' } },
        splitLine: { show: false },
        min: start,
        max: end,
      },
      ...
      series: [
        {
          ...
          barWidth: '90%',
          itemStyle: { color: isDark ? 'rgba(99,136,235,0.75)' : 'rgba(84,112,198,0.75)', borderRadius: [3, 3, 0, 0] },
          markLine: {
            silent: true,
            symbol: ['none', 'none'],
            lineStyle: { type: 'dashed' as const, width: 2 },
            label: { show: true, position: 'insideStartTop' as const, fontSize: 11, fontWeight: 'bold' as const },
            data: [
              { xAxis: mean, lineStyle: { color: '#fac858' }, label: { show: false } },
              { xAxis: median, lineStyle: { color: '#ee6666' }, label: { show: false } },
            ],
          } as any,
        },
        {
          ...
          areaStyle: { color: isDark ? 'rgba(115,192,222,0.08)' : 'rgba(115,192,222,0.15)' },
          tooltip: { show: false } as any,
        },
      ],
    };
  }, [distStats, textColor, isDark]);
  ...
  return (
    <main className={`flex flex-col px-4 md:px-12 w-full max-w-screen-2xl mx-auto ${
      viewMode === 'both' && !searchQuery ? 'pt-3 pb-12 items-center' : 'h-full overflow-hidden pt-3 pb-0'
    }`}>
      ...
      <div className="w-full flex items-center gap-2 mb-2 flex-shrink-0">
          ...
          <div className="flex gap-1 flex-shrink-0">
            <Tooltip>
              <TooltipTrigger asChild>
                <button onClick={() => setViewMode('both')} className={`p-1.5 rounded border transition-colors ${viewMode === 'both' ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-muted-foreground border-border hover:text-foreground hover:bg-muted'}`}><LayoutGrid className="w-3.5 h-3.5" /></button>
              </TooltipTrigger>
              <TooltipContent side="bottom">Chart and Table</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
                <button onClick={() => setViewMode('chart')} className={`p-1.5 rounded border transition-colors ${viewMode === 'chart' ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-muted-foreground border-border hover:text-foreground hover:bg-muted'}`}><BarChart2 className="w-3.5 h-3.5" /></button>
              </TooltipTrigger>
              <TooltipContent side="bottom">Chart only</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
                <button onClick={() => setViewMode('table')} className={`p-1.5 rounded border transition-colors ${viewMode === 'table' ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-muted-foreground border-border hover:text-foreground hover:bg-muted'}`}><Table2 className="w-3.5 h-3.5" /></button>
              </TooltipTrigger>
              <TooltipContent side="bottom">Table only</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
                <button onClick={() => setViewMode('distribution')} className={`p-1.5 rounded border transition-colors ${viewMode === 'distribution' ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-muted-foreground border-border hover:text-foreground hover:bg-muted'}`}><Activity className="w-3.5 h-3.5" /></button>
              </TooltipTrigger>
        ...
        {!searchQuery && (
          <div className="flex-1 flex gap-1 justify-center overflow-x-auto min-w-0" style={{ scrollbarWidth: 'none' }}>
            <button onClick={() => setSelectedDivision('overall')} className={cn('px-2 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap border-2 transition-all flex-shrink-0', selectedDivision === 'overall' ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-border text-muted-foreground hover:text-foreground hover:bg-muted')}>P4P</button>
            {divisions.map(key => (
              <button key={key} onClick={() => setSelectedDivision(key)} className={cn('px-2 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap border-2 transition-all flex-shrink-0', selectedDivision === key ? 'shadow-sm' : 'opacity-60 hover:opacity-90')} style={wcBadgeStyle(key)}>{WC_ABBREV[key] || key}</button>
            ))}
          </div>
        )}
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <button
            ...
            className={cn('h-7 px-2 text-xs rounded-lg border transition-colors whitespace-nowrap',
              activeOnly ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-muted-foreground border-border hover:text-foreground hover:bg-muted'
            )}
          >Active only</button>
          {searchQuery && (
            <button onClick={() => { setSearchQuery(''); setSearchResults([]); }} className="p-1 text-muted-foreground hover:text-foreground rounded transition-colors" title="Back">
              <ArrowLeft size={14} />
            </button>
          )}
          <div className="relative">
            <Search size={12} className="absolute left-2 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" />
            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search fighter…" className="pl-6 pr-2 h-7 text-xs rounded-lg border border-border bg-card focus:outline-none focus:ring-1 focus:ring-primary w-40" />
          </div>
        </div>
      </div>
      ...
      {(viewMode === 'chart' || viewMode === 'both') && (
        <Card className={`w-full ${viewMode === 'both' ? 'mb-6' : ''} ${viewMode === 'chart' ? 'flex-1 min-h-0 flex flex-col overflow-hidden' : ''}`}>
          <CardContent className={`p-1.5 md:p-1 ${viewMode === 'chart' ? 'flex-1 min-h-0 flex flex-col overflow-hidden' : ''}`}>
            {/* Title */}
            <div className="flex items-center justify-center mb-1">
              <TooltipProvider delayDuration={0}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <span className="font-bold text-base whitespace-nowrap cursor-help underline decoration-dotted decoration-muted-foreground/50">UFC Elo Ratings</span>
                  </TooltipTrigger>
                  <TooltipContent className="max-w-xs text-xs p-2" side="bottom">
                    <div className="space-y-1">
                      <div><span className="font-semibold">Base Elo:</span> 1500</div>
                      <div><span className="font-semibold">K-factor:</span> 40 (debut) → 24 (30+ fights), linear taper</div>
                      <div><span className="font-semibold">Finish bonus:</span> +10 pts</div>
                    </div>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            ...
            >
              <ReactECharts
                ref={chartRef}
                option={chartOption}
                style={{ width: '100%', height: viewMode === 'chart' ? '100%' : '480px' }}
                notMerge
                lazyUpdate
            ...
            <div className="flex flex-wrap justify-center gap-x-4 gap-y-1.5 pt-2 pb-1">
              {displayFighters.map((f, i) => {
                const isActive = lockedFighter === null || lockedFighter === f.name;
                const lgKey = f.name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[.'\u2011]/g, '').toLowerCase().trim();
                const lgChamp = championMap.get(lgKey);
                const lgHeadshot = lgChamp?.headshotUrl ?? candidateHeadshots.get(lgKey);
                const lgTott = candidateTott.get(lgKey);
                const lgWins = f.history.filter(h => h.result === 'W').length;
                const lgLosses = f.history.filter(h => h.result === 'L').length;
                const lgDraws = f.history.filter(h => h.result === 'D').length;
                return (
                  <TooltipProvider key={f.name} delayDuration={400}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div
                          onClick={() => setLockedFighter((prev) => (prev === f.name ? null : f.name))}
                          className={`flex items-center gap-1.5 text-xs transition-opacity select-none cursor-pointer hover:opacity-100 ${
                            isActive ? 'opacity-100 text-foreground' : 'opacity-30 text-muted-foreground'
                          }`}
                        >
                          <div
                      ...
                      <TooltipContent side="top" className="p-0 overflow-hidden bg-card text-card-foreground border border-border shadow-lg">
                        <div className="flex gap-3 p-3 min-w-[200px] max-w-[240px]">
                          <div className="w-12 h-12 rounded-full overflow-hidden bg-muted flex items-center justify-center flex-shrink-0 border border-border">
                            {lgHeadshot ? <img src={lgHeadshot} alt={f.name} className="w-full h-full object-cover object-top" /> : <User className="w-6 h-6 text-muted-foreground" />}
                          </div>
                          <div className="flex flex-col gap-0.5 text-xs min-w-0 justify-center">
                            <div className="font-bold text-sm truncate leading-tight">{f.name}</div>
                            {WC_ABBREV[f.primaryWeightClass] && (
                              <span className="text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded border w-fit" style={wcBadgeStyle(f.primaryWeightClass)}>{WC_ABBREV[f.primaryWeightClass]}</span>
                            )}
                            {/* Always show Elo with golden number only */}
                            <div className="flex items-center gap-1 my-0.5">
                              <span className="text-[10px] text-foreground/70">Elo:</span>
                              <span className="inline-flex items-center px-1.5 py-0.5 rounded-full border border-yellow-400/60 bg-yellow-400/10 font-bold text-xs text-yellow-500 dark:text-yellow-400">{f.currentElo}</span>
                              <span className="text-[10px] text-foreground/50">/ {f.peakElo}</span>
                            </div>
                            {lgTott ? (
                              <>
                                {lgTott.height && <div><span className="font-medium">Height:</span> {lgTott.height}</div>}
                                {lgTott.weight && <div><span className="font-medium">Weight:</span> {lgTott.weight}</div>}
                                {lgTott.reach && <div><span className="font-medium">Reach:</span> {lgTott.reach}</div>}
                                {lgTott.stance && <div><span className="font-medium">Stance:</span> {lgTott.stance}</div>}
                                {lgTott.dob && <div><span className="font-medium">Born:</span> {formatDob(lgTott.dob)}</div>}
                              </>
                            ) : (
                              <div>{lgWins}-{lgLosses}{lgDraws > 0 ? `-${lgDraws}` : ''}</div>
                            )}
                          </div>
                        </div>
                      </TooltipContent>
      ...
      {(viewMode === 'table' || viewMode === 'both') && (
        <Card className={`w-full ${viewMode === 'table' ? 'flex-1 min-h-0 flex flex-col overflow-hidden' : ''}`}>
          <CardContent className={`p-0 md:p-0 ${viewMode === 'table' ? 'flex-1 min-h-0 flex flex-col overflow-hidden' : ''}`}>
            <div
              ref={tableWrapperRef}
              className={`overflow-x-auto ${viewMode === 'table' ? 'flex-1 min-h-0 overflow-y-hidden' : ''}`}
            >
              <table className={`w-full text-sm${viewMode === 'table' ? ' h-full' : ''}`}>
                <thead className="sticky top-0 bg-card z-10">
                  <tr className="border-b border-border text-left">
                    <th className="py-3 px-3 font-semibold w-8 text-center text-foreground">#</th>
                    <th className="py-3 px-3 font-semibold text-left">Fighter</th>
                ...
                <tbody>
                  {displayFighters.map((f, i) => {
                    ...
                    const losses = f.history.filter((h) => h.result === 'L').length;
                    const draws = f.history.filter((h) => h.result === 'D').length;
                    const record = draws > 0 ? `${wins}-${losses}-${draws}` : `${wins}-${losses}`;
                    const wlRatioRaw = losses === 0 ? (wins > 0 ? Infinity : 0) : wins / losses;
                    const wlRatio = losses === 0
                      ? (wins > 0 ? '∞' : '—')
                      : Number.isInteger(wlRatioRaw) ? String(wlRatioRaw) : wlRatioRaw.toFixed(2);
                    const { l5Str, streak } = f as typeof f & { l5Str: string; streak: number };
                    const strkDisplay = streak === 0 ? '—' : streak > 0 ? `W${streak}` : `L${Math.abs(streak)}`;
                    const strkColor = streak > 0 ? 'text-green-500' : streak < 0 ? 'text-red-500' : 'text-foreground';
                    return (
                      <tr
                        key={f.name}
                        className="border-b border-border/50 hover:bg-muted/30 last:border-b-0"
                        style={viewMode === 'table' ? { height: tableRowHeight ?? undefined } : undefined}
                      >
                        <td className="py-2 px-3 text-center text-foreground">{i + 1}</td>
                        <td className="py-2 px-3 font-medium text-left">
                          <TooltipProvider delayDuration={400}>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <button
                                  onClick={() => {
                                    ...
                                    setSelectedChampion(champ ? { ...champ, eloHistory: f.history } : { name: f.name, eloHistory: f.history });
                                  }}
                              ...
                              <TooltipContent side="right" className="p-0 overflow-hidden bg-card text-card-foreground border border-border shadow-lg">
                                {(() => {
                                  const normKey = f.name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[.'\u2011]/g, '').toLowerCase().trim();
                                  const champEntry = championMap.get(normKey);
                                  const headshotUrl = champEntry?.headshotUrl ?? candidateHeadshots.get(normKey);
                                  ...
                                  return (
                                    <div className="flex gap-3 p-3 min-w-[200px] max-w-[240px]">
                                      <div className="w-14 h-14 rounded-full overflow-hidden bg-muted flex items-center justify-center flex-shrink-0 border border-border">
                                        {headshotUrl
                                          ? <img src={headshotUrl} alt={f.name} className="w-full h-full object-cover object-top" />
                                          : <User className="w-6 h-6 text-muted-foreground" />}
                                      </div>
                                      <div className="flex flex-col gap-0.5 text-xs min-w-0 justify-center">
                                        <div className="font-bold text-sm truncate leading-tight">{f.name}</div>
                                        {WC_ABBREV[f.primaryWeightClass] && (
                                          <span className="text-[10px] font-semibold uppercase px-1.5 py-0.5 rounded border w-fit" style={wcBadgeStyle(f.primaryWeightClass)}>
                                            {WC_ABBREV[f.primaryWeightClass]}
                                          </span>
                                        )}
                                        {f.currentElo !== undefined && (
                                          <div className="flex items-center gap-1 my-0.5">
                                            <span className="text-[10px] text-foreground/70">Elo:</span>
                                            <span className="inline-flex items-center px-1.5 py-0.5 rounded-full border border-yellow-400/60 bg-yellow-400/10 font-bold text-xs text-yellow-500 dark:text-yellow-400">{f.currentElo}</span>
                                            {f.peakElo !== undefined && <span className="text-[10px] text-foreground/50">/ {f.peakElo}</span>}
                                          </div>
                                        )}
                                        {tott?.height && <div><span className="font-medium">Height:</span> {tott.height}</div>}
                                        {tott?.weight && <div><span className="font-medium">Weight:</span> {tott.weight}</div>}
                          ...
                          {selectedDivision === 'overall' && WC_ABBREV[f.primaryWeightClass] && (
                            <TooltipProvider delayDuration={0}>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <span
                                ...
                                <TooltipContent>{WC_FULL_NAME[f.primaryWeightClass] || f.primaryWeightClass}</TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          )}
                        ...
                        <td className="py-2 px-3 text-center font-bold">{f.currentElo}</td>
                        <td className="py-1 px-3 text-center">{f.peakElo}</td>
                        <td className="py-1 px-3 text-center">
                          {f.peakDate ? (() => {
                            const peakFight = f.history.find((h) => h.date === f.peakDate);
                            const dateStr = new Date(f.peakDate).toLocaleDateString();
                            const eventName = peakFight?.event;
                            const inner = peakFight?.url ? (
                              <a href={peakFight.url} target="_blank" rel="noopener noreferrer" className="hover:underline hover:text-primary">
                                {dateStr}
                              </a>
                            ) : <span>{dateStr}</span>;
                            return eventName ? (
                              <TooltipProvider delayDuration={0}>
                                <Tooltip>
                                  ...
                                  <TooltipContent side="top" className="text-xs">{eventName}</TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                            ) : inner;
                          })() : '—'}
                        </td>
                        <td className="py-1 px-3 text-center">{f.fights}</td>
                        <td className="py-1 px-3 text-center text-foreground">{l5Str}</td>
                        <td className={`py-1 px-3 text-center font-semibold ${strkColor}`}>{strkDisplay}</td>
                        <td className="py-1 px-3 text-center text-foreground">{wlRatio}</td>
                      </tr>
                    );
                  })}
                </tbody>
        ...
        <Card className="w-full flex-1 min-h-0 flex flex-col overflow-hidden">
          <CardContent className="p-3 md:p-4 flex-1 min-h-0 flex flex-col gap-2 overflow-y-auto">
            ...
            <div className="flex justify-center gap-6 text-xs text-muted-foreground">
              <span className="flex items-center gap-1.5"><span className="inline-block w-5 h-0.5 bg-[#fac858]" /> Mean</span>
              <span className="flex items-center gap-1.5"><span className="inline-block w-5 h-0.5 bg-[#ee6666]" /> Median</span>
              <span className="flex items-center gap-1.5"><span className="inline-block w-5 h-0.5 rounded bg-[#73c0de]" style={{ height: 2 }} /> KDE curve</span>
            </div>
            ...
            {distStats && (
              <div className="grid grid-cols-4 sm:grid-cols-7 gap-2 w-full">
                {([
                  { label: 'Fighters', value: distStats.n, tooltip: 'Fighters with ≥5 UFC fights' },
                  { label: 'Mean', value: Math.round(distStats.mean) },
                  { label: 'Median', value: Math.round(distStats.median) },
                  { label: 'Std Dev', value: Math.round(distStats.std) },
                  { label: 'Q1 / Q3', value: `${distStats.q1} / ${distStats.q3}` },
                  { label: 'Min', value: distStats.min },
                  { label: 'Max', value: distStats.max },
                ] as { label: string; value: number | string; tooltip?: string }[]).map(({ label, value, tooltip }) => {
                  const cell = (
                    <div className={cn('rounded-lg border border-border bg-muted/30 px-3 py-2 text-center', tooltip && 'cursor-help')}>
      ...
      {searchQuery && (
        <div className="w-full flex-1 overflow-y-auto">
          {isSearching ? (
            ...
            <p className="text-center text-muted-foreground py-12 text-sm">No fighters found matching &ldquo;{searchQuery}&rdquo;</p>
          ) : (
                  ...
                  {searchResults.map(result => (
                    <li key={result.name}>
                      <button
                        ...
                        onClick={() => {
                          const key = result.name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[.'\u2011]/g, '').toLowerCase().trim();
                          const champEntry = championMap.get(key);
                          setSelectedChampion(champEntry
                            ? { ...champEntry, eloHistory: eloLookup(result.name) }
                            : { name: result.name, eloHistory: eloLookup(result.name) });
                        }}
                      >
                        <div className="w-12 h-12 rounded-full overflow-hidden bg-muted flex-shrink-0 flex items-center justify-center">
                          {result.headshot
                            ? <img src={result.headshot} alt={result.name} className="w-full h-full object-cover object-top" />
                            : <User className="w-6 h-6 text-muted-foreground" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <p className="font-semibold text-sm truncate">{result.name}</p>
                            {result.weightClass && WC_ABBREV[result.weightClass] && (
                              <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded border flex-shrink-0" style={wcBadgeStyle(result.weightClass)}>{WC_ABBREV[result.weightClass]}</span>
                            )}
                          </div>
                          <div className="flex items-center gap-2 mt-0.5">
                            {result.record && <span className="text-xs text-muted-foreground">{result.record}</span>}
                            {result.fights !== undefined && <span className="text-xs text-muted-foreground">· {result.fights} UFC fights</span>}
                          </div>
                        </div>
                          ...
                          {result.eloStatus === 'loading' ? (
                            ...
                            <div className="flex items-center gap-3">
                              <div>
                                ...
                                <p className="font-bold text-sm">{result.currentElo}</p>
                              </div>
                              {result.peakElo !== undefined && (
                                <div>
                                  <p className="text-[10px] text-muted-foreground">Peak</p>
                                  <p className="font-semibold text-sm">{result.peakElo}</p>
                                </div>
                              )}
                            </div>
                          ) : null}
