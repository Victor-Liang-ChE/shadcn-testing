...
function calculateActivityGamma(comps: CompoundData[], x: number[], T: number, pkg: FluidPackageType, p: any): [number, number]|null {
    switch(pkg){
        case 'nrtl': { const R_cal = 1.9872; const t12 = p.Aij/(R_cal*T) + p.Bij / R_cal, t21 = p.Aji/(R_cal*T) + p.Bji / R_cal, G12 = Math.exp(-p.alpha * t12), G21 = Math.exp(-p.alpha * t21); const d1 = x[0] + x[1] * G21, d2 = x[1] + x[0] * G12; if (d1 === 0 || d2 === 0) return null; return [Math.exp(x[1] ** 2 * (t21 * (G21 / d1) ** 2 + t12 * G12 / d2 ** 2)), Math.exp(x[0] ** 2 * (t12 * (G12 / d2) ** 2 + t21 * G21 / d1 ** 2))]; }
        case 'wilson': { if(!comps[0].wilsonParams||!comps[1].wilsonParams) return null; const R_cal = 1.9872; const V1 = comps[0].wilsonParams.V_L_m3mol, V2 = comps[1].wilsonParams.V_L_m3mol; const L12 = (V2 / V1) * Math.exp(-(p.Aij / (R_cal * T) + p.Bij / R_cal)), L21 = (V1 / V2) * Math.exp(-(p.Aji / (R_cal * T) + p.Bji / R_cal)); return [Math.exp(-Math.log(x[0] + L12 * x[1]) + x[1] * (L12 / (x[0] + L12 * x[1]) - L21 / (x[1] + L21 * x[0]))), Math.exp(-Math.log(x[1] + L21 * x[0]) - x[0] * (L12 / (x[0] + L12 * x[1]) - L21 / (x[1] + L21 * x[0])))];}
